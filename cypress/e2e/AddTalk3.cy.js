...
function createTalk(day, talkTitle, startDate, startTime, endDate, endTime) {
    ...
    cy.get('.hours').eq(1).clear().type(startTime.hours);
    cy.get('.minutes').eq(1).clear().type(startTime.minutes);
    cy.get('#endDate').click().clear().type(endDate);
    cy.get('.hours').eq(4).clear().type(endTime.hours);
    cy.get('.minutes').eq(4).clear().type(endTime.minutes);
  ...
  function editTalk(talkTitle, newStartDate, newStartTime, newEndDate, newEndTime) {
    ...
    // Assuming there is a common class for the edit button, adjust the selector accordingly
    ...
    cy.get('.glyphicon-edit').each(($talkTitle, index) => {
      ...
      cy.get('#startDate').click().clear().type(newStartDate);
      cy.get('#endDate').click().clear().type(newEndDate);
      cy.get('.hours').eq(1).clear().type(newStartTime.hours);
      cy.get('.minutes').eq(1).clear().type(newStartTime.minutes);
      cy.get('.hours').eq(4).clear().type(newEndTime.hours);
      cy.get('.minutes').eq(4).clear().type(newEndTime.minutes);
      cy.get('fieldset > :nth-child(3) > .btn').should('be.visible').click();
    });
  }
  ...
  const talkTimes = {
    ...
    wednesday: [
      // Define timings for Wednesday talks
    ],
    thursday: [
      // Define timings for Thursday talks
    ],
  };
